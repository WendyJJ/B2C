...
def typeindex(request):
    ...
    # 遍历查询,为每个结果对象追加一个pname属性,目的用于缩进标题
    for ob in list:
...
def goodsindex(request,pIndex=1):
    ...
    if request.GET.get('typename','') != '':
        list = list.filter(typename=request.GET['typename'])
        where.append('typename='+request.GET.get('typename'))
    #print(where)
    # 传入数据和页大小来创建分页对象
    p = Paginator(list, 4)
    # 判断页号没有值时初始化为1
    ...
    # 正则判断
    path = request.path
    p = re.compile(r'/\w+/goods\d+')
    path_flag= p.match(path)
    if path_flag == None:
        path='&'

    #封装分页信息
    context = {'goodslist':list2,'plist': plist, 'pIndex': pIndex, 'where':where,'path':path,'yy':yy}
...
def goodsinsert(request):
    try:
        ...
        im.thumbnail((50, 50))
        # 把缩放后的图像用jpeg格式保存:
        im.save("./static/goods/sx_"+filename, None)
